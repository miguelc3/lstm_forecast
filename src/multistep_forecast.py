...
# === functions ===
def create_sequences_multistep(data, n_steps, n_preds):
    data_t = data.to_numpy()
    X = []
    y = []

    for i in range(len(data_t) - n_steps - n_preds):
      row = [a for a in data_t[i:i+n_steps]]
      X.append(row)

      label = data_t[i+n_steps: i+n_steps+n_preds][:, 0]
      y.append(label)
...
def train_model(X, y, X_val, y_val, n_steps, n_preds=1):
    ...
    checkpoint_filepath = 'cps/best_model_multistep.h5'
...
def postprocess_input(X, mean, std):
    X = (X * std) + mean
    return X


def plot_predictions_multistep(model, X_test, y_test, i, mean_ref, std_ref):

    X_pred = X_test[i].reshape(1, X_test.shape[1], X_test.shape[2])
    preds = model.predict(X_pred).flatten().tolist()

    y_t = y_test[i]

    # preprocess preds to actual scale
    preds = [postprocess_output(i, mean_ref, std_ref) for i in preds]
    y_t = [postprocess_output(i, mean_ref, std_ref) for i in y_t.tolist()]

    er = mse(y_t, preds)

    x_prev = X_pred[0][:, 0]
    x_prev = [postprocess_input(i, mean_ref, std_ref) for i in x_prev.tolist()]

    plt.figure(figsize=(12, 8))
    plt.plot(range(len(x_prev)), x_prev, label='previous values')
    plt.plot(range(len(x_prev) - 1, len(x_prev) + len(y_t) - 1), preds, label='Predictions', alpha=.7)
    plt.plot(range(len(x_prev)-1, len(x_prev) + len(y_t) - 1), y_t, label='Actual values')
    plt.axvline(len(x_prev)-1, c='r', linestyle='--')
    plt.legend()
...
n_steps = 5
n_preds = 6
X, y = create_sequences_multistep(df, n_steps, n_preds)
...
model = train_model(X_train, y_train, X_val, y_val, n_steps, n_preds)

plot_predictions_multistep(model, X_train, y_train, 100, mean_ref, std_ref)
plot_predictions_multistep(model, X_test, y_test, 50, mean_ref, std_ref)
